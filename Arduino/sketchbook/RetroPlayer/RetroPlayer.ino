...
void RetroPlayer<digitalIns, analogIns, digitalOuts>::setup()
{
    ...
    for (byte i = 1; i < NUM_DIGITAL; i++)
    { //Start at 1 to miss first
...
void RetroPlayer<digitalIns, analogIns, digitalOuts>::check_dig_inputs()
{
    ...
    for (byte i = 0; i < NUM_DIGITAL; i++)
    {
        ...
        if (debounce[i] == 1)
        {
            ...
            CALL_MEMBER_FN(*this, inputCallbacks[i])
            (digitalInStates[i]);              // Use #define macro to call input functions
...
void RetroPlayer<digitalIns, analogIns, digitalOuts>::set_outputs()
{
    for (byte i = 0; i < NUM_OUTPUTS; i++)
    {
        //TODO If output set to 3 then keep as is
...
template <byte digitalIns, byte analogIns, byte digitalOuts>
void RetroPlayer<digitalIns, analogIns, digitalOuts>::maintenence_mode()
{ // Cancel analog power setup so i2c can be used
...
template <byte digitalIns, byte analogIns, byte digitalOuts>
void RetroPlayer<digitalIns, analogIns, digitalOuts>::normal_mode()
{ // Setup analogues to read
...
{
    switch (powerState)
    {                  // State machine to control power up sequence
    ...
    case off:              // Low power state
        ...
        if (volSwitch == 0)
        { // If switch is off, go back to sleep.
            display = 0;
            break;
        }
